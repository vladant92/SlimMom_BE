...
const addProduct = async (req, res, next) => {
  ...
  const allDaysInfo = await Promise.all(
    user.days.map((dayId) => Day.findById(dayId).exec())
  );
...
const deleteProduct = async (req, res) => {
  ...
  const neededDay = req.user.days.find((day) => day.toString() === dayId);
  if (!neededDay) {
    return res.status(404).send({ message: "Day not found" });
  }
  const product = day.eatenProducts.find(
...
const addNewProduct = async (req, res, next) => {
  ...
  const checkItem = await Product.find({
    title: productName,
  });
  if (checkItem.length > 0) {
    return res
      .status(200)
      .send({ message: "Such a product is already in the database" });
  }
  ...
  if (Object.keys(newProduct).length) {
    return res
      .status(201)
      .send({ message: "The product has been added to the database" });
  }
};
